...
class UsuarioManager(BaseUserManager):
    ...
    def create_user(self, email, nombre, tipo, password=None):
        ...
        nuevoUsuario = self.model(usuario_correo=email, usuario_nombre=nombre, usuario_tipo=tipo)
    ...
    def create_superuser(self, usuario_correo, usuario_nombre, usuario_tipo, password):
        """Creacion de un super usuario para que pueda acceder al panel administrativo y algunas opciones mas"""
        usuario = self.create_user(usuario_correo, usuario_nombre, usuario_tipo, password)
